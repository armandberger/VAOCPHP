...
<html>
    ...
    {
        //ne pas mettre cette fonction dans le .js sinon quand le tour est en cours, la fonction n'est pas disponible
        //alert("callQuitter");
        formPrincipale = document.getElementById("principal");
        formPrincipale.action = "index_s.php";
...
/* 	if (!headers_sent())
        ...
        if (strlen($message) > 0)
        {
            ...
            $message.= "<button alt=\"quitter\" id='id_quitter' name='id_quitter' class=\"btn btn-light\" type='button' value='submit' onclick=\"javascript:location.href='index_s.php';\" />Quitter</button>";
        ...
        if (1 == $row_role->FL_MISEAJOUR)
        {
            ...
            echo "<button class=\"btn btn-light bouton\" alt=\"retour &agrave; l'ecran g&eacute;n&eacute;ral\" id=\"id_quitter\" name=\"id_quitter\" type=\"button\" value=\"submit\" onclick=\"javascript:callQuitter();\" />Quitter</button>";
            echo "</div>";
            echo "</form></body></html>";
            die;
        }
        ...
        Debug_Logger::getInstance()->timeAddPoint('avant le traitement des commandes');
        //echo "cache_control = ".filter_input(INPUT_SERVER, 'HTTP_CACHE_CONTROL');
        ...
        //if (FALSE == empty($_SESSION['action'])) {echo "action=".$_SESSION['action']."</BR>";}
        ...
        if (TRUE == isset($_SESSION['action']) && $_SESSION['action']=="OK")
        {
            echo "navigateur =".get_nom_navigateur($_SERVER['HTTP_USER_AGENT']);
            $_SESSION['action']="";
            ...
            if (TRUE == isset($id_supprimerallera) && TRUE == is_numeric($id_supprimerallera))
            {
                ...
                //s'il y a un ordre precedent, il faut remettre l'ordre_suivant a -1
                ...
                //s'il y a des ordres suivants, il faut egalement tous les supprimer
            ...
            if (TRUE == isset($id_patrouillera) && TRUE == is_numeric($id_patrouillera))
            {
                ...
                if (TRUE == isset($_REQUEST["id_destination_patrouille_" . $id_patrouillera]) && $_REQUEST["id_destination_patrouille_" . $id_patrouillera]>0 )
                {
                    ...
                    {
                        //echo "get_magic_quotes_gpc() ON";
                        $requete.= mysql_real_escape_string($_REQUEST["id_destination_patrouille_" . $id_patrouillera], $db) . "'";
                    }
                     */
                    //echo $requete;
                    //$res_destination = mysql_query($requete, $db);
            ...
            if (TRUE == isset($id_ligne_ravitaillement) && TRUE == is_numeric($id_ligne_ravitaillement))
            {
                ...
                //echo "id_ligne_ravitaillement=".$id_ligne_ravitaillement;
                /*
                //on recherche le dernier ordre deja donne a ce tour, hors combat ou retraite, s'il exite, cela devient un ordre_suivant
                ...
                */
                if (TRUE == isset($_REQUEST["id_destination_mouvement_" . $id_ligne_ravitaillement]) && $_REQUEST["id_destination_mouvement_" . $id_ligne_ravitaillement]>0 )
                {
                    ...
                    if (get_magic_quotes_gpc())
                    {
                        // le cas sur Free alors que c'est une faille de securite...
                        //echo "get_magic_quotes_gpc() OFF";
                        $requete.= $_REQUEST["id_destination_mouvement_" . $id_ligne_ravitaillement] . "'";
                    }
                    else
                    {
                        //echo "get_magic_quotes_gpc() ON";
                        $requete.= mysql_real_escape_string($_REQUEST["id_destination_mouvement_" . $id_ligne_ravitaillement], $db) . "'";
                    }
                    echo $requete;
                     */
                    //$res_destination = mysql_query($requete, $db);
            ...
            {
                //reduction d'un depot en creant un convoi, toujours un ordre unique jamais en suivant
                //insertion du nouvel ordre
                $requete = "INSERT INTO tab_vaoc_ordre(`ID_ORDRE` ,`ID_PION` ,`ID_PION_DESTINATION` ,`ID_PION_CIBLE`, `ID_PARTIE` ,`I_TOUR` ,`I_TYPE` ,`S_MESSAGE` ,";
                $requete.="`I_DISTANCE` ,`I_DIRECTION` ,`ID_NOM_LIEU` ,`I_HEURE` ,`I_DUREE`) VALUES (";
        ...
        Debug_Logger::getInstance()->timeAddPoint('apres le traitement des commandes');
        ?>
        <div class="container">
        <form method="post" id="principal" action="<?php echo $_SERVER['PHP_SELF']; ?>" role="form">
            <?php
            if (empty($tri_liste))
            {
                //lorsque l'on arrive sur la page, on est forcement sur la premiere page des messages
            ...
            <div class="row justify-content-center d-none d-md-flex">
                <div class="col-12 text-center">
                    <a href="nojs.htm" onclick="javascript:callPage('vaoccampagne_s.php');return false;">
                ...
                <div class="col-12 text-center" style='position:relative; top:-60px; height: 0'>
                    <?php
                    if ($idVictoire >= 0) //la partie est terminee
                    {
                        echo "<button alt=\"victoire\" id='id_victoire' name='id_victoire' class=\"btn btn-info bouton\" type='button' value='submit' onclick=\"javascript:callVictoire(); return false;\" >Victoire</button>&nbsp;";
                        echo "<button alt=\"historique\" id='id_historique' name='id_historique' class=\"btn btn-info bouton\" type='button' value='submit' onclick=\"javascript:callHistorique(); return false;\" >Historique</button>";
                    }
                    else
                    {
                        echo "<button alt=\"aide\" id='id_aide' name='id_aide' class=\"btn btn-info bouton\" type='button' value='submit' onclick=\"javascript: window.open('aide_s.php'); return false;\" >Aide</button>&nbsp;";
                        echo "<button alt=\"retour a l'ecran general\" id='id_quitter' name='id_quitter' class=\"btn btn-light bouton\" type='button' value='submit' onclick=\"javascript:callQuitter();\" >Quitter</button>";
                        //s'il y a une bataille en cours, on met un gros bouton d'accès rapide
                        if ($row_pion_chef->ID_BATAILLE>=0)
                        {
                            echo "<button alt=\"bataille\" id='id_afficher_bataille' name='id_afficher_bataille' class=\"btn btn-danger bouton\" type='button' value='submit' onclick=\"javascript:callBataille();\" />Bataille</button>";
                        }
                    }
                    ?>
                </div>
            ...
            <div class="row justify-content-center">
                <div class="col-12 text-center">
                    ...
                    else
                    {
                        ...
                        echo "<h5>Nous sommes le " . $row_role->DATE_PARTIE . ". Cette campagne est pr&eacute;vue jusqu'au " . $row_role->DATE_FIN . "</h5>";
                        echo "<h5>Actuellement, le temps est " . $row_role->S_METEO . ". Il fait jour de " . $row_role->H_JOUR . "h00 &agrave; " . $row_role->H_NUIT . "h00.</h5>";
                    }
                    ?>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-12">			
                    ...
                    <div id="editor" style="margin-left:-0px;"></div>
                </div>
                <div class="col-12">
                ...
                <div class="col-12">
                    ...
                    echo "<button alt=\"envoyer le message\" id=\"id_envoie_message\" name=\"id_envoie_message\" "
                    . "type= \"button\" value=\"submit\" class=\"btn btn-light bouton\" "
                            . "onclick=\"javascript:callDonnerOrdreUnite(" . $row_pion_chef->ID_PION . ",'id_envoyermessagea');\">Envoyer le message</button>";
                    echo "<div class=\"input-group\">";
                    echo "&nbsp;<div class=\"input-group\">";
                            echo "<div class=\"input-group-prepend\"><span class=\"input-group-text\">&nbsp;&agrave;&nbsp;&nbsp;</span></div>";
                            echo "<select class=\"custom-select\" id=\"id_destinataire_message\" name=\"id_destinataire_message\" size=1>";
                            if (0 == $fl_demmarage)
                            {
                    ...
                    echo "</div>";
                    echo "&nbsp;<div class=\"input-group\">";
                        echo "<div class=\"input-group-prepend\"><span class=\"input-group-text\">&nbsp;situ&eacute; &agrave;&nbsp;</span></div>";
            ...
            <div class="row">
                <div class="col-md-12 hidden-xs hidden-sm" >
                    <h3 id="troupes">Troupes
                        <button type="button" class="btn btn-light bouton" data-toggle="modal" data-target="#id_fenetre_reorganise">Trier</button>
                    </h3>
                </div>
            </div>
            <?php
            Debug_Logger::getInstance()->timeAddPoint('avant les unites');
            $i_tri=0;
            ...
            $requete.=" B_CAVALERIE_DE_LIGNE, B_CAVALERIE_LOURDE, B_GARDE, B_VIEILLE_GARDE, B_QG, I_NIVEAU_FORTIFICATION, S_ORDRE_COURANT, DATE_FORMAT(DT_DERNIER_MESSAGE,'%W %e %M %Y %H:%i') AS DATE_DERNIER_MESSAGE ";
            $requete.=" FROM tab_vaoc_pion";
                    ...
                    else
                    {
                        if (1 == $row_pion->B_DEPOT)
                        {
                            AfficherDepot($db, $fl_demmarage, $id_partie, $id_pion_role, $row_pion->ID_PION, 
                                            ...
                                            $row_pion->S_POSITION, $i_tour, $row_pion->DATE_DERNIER_MESSAGE, $numPage);
                            echo "<hr style=\"border-top: 5px solid white;\" />";
                        ...
                        else
                        {
                            if (1 == $row_pion->B_PONTONNIER)
                            {
                                AfficherPontonnier($db, $fl_demmarage, $id_partie, $id_pion_role, $row_pion->ID_PION, 
                                                    ...
                                                    $row_pion->B_DETRUIT, $row_pion->S_POSITION, $i_tour, $row_pion->DATE_DERNIER_MESSAGE, $numPage);
                                echo "<hr style=\"border-top: 5px solid white;\" />";
                                    ...
                                    if ((1 == $row_pion->B_CONVOI && $row_pion->C_NIVEAU_DEPOT <> 'D') || 1==$row_pion->B_BLESSES || 1==$row_pion->B_PRISONNIERS || 1 == $row_pion->B_RENFORT)
                                    {
                                        ...
                                        if (0 == $row_pion->B_DETRUIT)
                                        {
                                            ...
                                            AfficherConvoi($db, $fl_demmarage, $id_partie, $id_pion_role, $row_pion->ID_PION, 
                                                                            ...
                                                                            $i_tour, $row_pion->DATE_DERNIER_MESSAGE, $numPage);
                                            echo "<hr style=\"border-top: 5px solid white;\" />";
                                            $ligne_tri .=ligneDrag($i_tri,$row_pion->ID_PION, $row_pion->S_NOM, "ravitaillement-26.png");
                                            $ligne_tri .=ligneDrop($i_tri++);
                                        }
                                    ...
                                    else
                                    {
                                        if ($row_pion->I_INFANTERIE_INITIALE>0 || $row_pion->I_CAVALERIE_INITIALE>0)
                                        {
                                            AfficherDivision($db, $fl_demmarage, $id_partie, $id_pion_role, $row_pion->ID_PION, 
                                                                ...
                                                                $row_pion->I_PATROUILLES_MAX, $row_pion->I_NIVEAU_FORTIFICATION, $row_pion->DATE_DERNIER_MESSAGE, $numPage);
                                            echo "<hr style=\"border-top: 5px solid white;\" />";
                                        ...
                                        else
                                        {
                                            AfficherArtillerie($db, $fl_demmarage, $id_partie, $id_pion_role, $row_pion->ID_PION, 
                                                            ...
                                                            $row_pion->I_ARTILLERIE_INITIALE, $row_pion->S_POSITION, $row_pion->ID_BATAILLE, $i_tour, $row_pion->DATE_DERNIER_MESSAGE, $numPage);
                                            echo "<hr style=\"border-top: 5px solid white;\" />";
            ...
            Debug_Logger::getInstance()->timeAddPoint('apres les unites');
            ?>            
            <div class="row justify-content-center">
                <div class="col-12 text-center">
                    <button name="id_campagne" class="btn btn-info bouton"
                           id="id_campagne" onclick="javascript: callPage('vaoccampagne_s.php'); return false;"
                           type="button" alt="campagne" >Campagne</button>
                ...
                <div class="modal-dialog modal-dialog-centered" role="document">
                    <div class="modal-content reorganisation-modal">
                        ...
                        <div class="modal-footer">
                            <button name="id_validerReorg" class="btn btn-light bouton" data-dismiss="modal" id="id_validerReorg" onclick="javascript:Trier(); return false;"
                                    data-dismiss="modal" type="button" alt="trier" value="submit"/>Trier</button>&nbsp;
                            <button name="id_annulerReorg" class="btn btn-light bouton" data-dismiss="modal"
                                    id="id_annulerReorg" type="button" alt="quitter" value="submit"/>Quitter</button>
                        </div>
                    </div>
                </div>
            </div>	
        ...
        $(document).ready(function () {
            //appel de la barre des messages
        <?php
            //exemple pour debug http://localhost/vaoc/vaocqg_messages_s.php?id_pion_role=220&id_partie=8&tri_liste=DT_ARRIVEE&ordre_tri_liste=DESC&pageNum_recus=1&nombre_messages_pages=100
            echo "$('#ouvrirmessages').on('click', function () {";
                echo "$('#SidebarMessages').load('vaocqg_messages_s.php', ";
                    echo "{id_login:'".$id_login."', id_pion_role:".$id_pion_role.", id_partie:".$id_partie.",tri_liste:'".$tri_liste."', ordre_tri_liste:'".$ordre_tri_liste."', pageNum_recus:".$pageNum_recus.", nombre_messages_pages :-1}";
                    echo ");";
                echo "$('#SidebarMessages').css('width', '100%');";
            echo "});";

            //case à cocher pour ne voir que les unites combattives visibles
            //exemple pour debug http://localhost/vaoc/vaocqg_unites_visibles_s.php?id_role=14&id_partie=8&combattif_visible=true    
            echo "$('#combattives_visibles').on('click', function () {";
                //echo "alert($('#combattives_visibles').is(':checked'));";
                echo "$('#div_unites_visibles').load('vaocqg_unites_visibles_s.php', {id_role:".$id_role.", id_partie:".$id_partie.",combattif_visible:$('#combattives_visibles').is(':checked')}";
                echo ");\r\n";
            echo "});";
        ?>
        });
        ...
        <!-- Sidebar pour les messages -->
        <button class="btn btn-light bouton boutonsidebarmessages" type="button" alt="ouvrir les messages" id="ouvrirmessages">Messages</button>
        <div id="SidebarMessages" class="sidebar" style='text-align:center;'><img alt='attente' src='images/giphy.gif' /></div>
	...
	?>
        ...
        /*
            ...
            echo "<div class=\"row\">";
                echo "<div class=\"col-12\"><h2>Performance</h2></div>";
            echo "</div>";
            echo Debug_Logger::getInstance()->getTimeLineString();
            echo "<div class=\"row\">";
                echo "<div class=\"col-12\"><h2>Cumul</h2></div>";
            echo "</div>";
            echo Debug_Logger::getInstance()->getTimeMesureString();
         */
        ?>
    </body>
</html>
